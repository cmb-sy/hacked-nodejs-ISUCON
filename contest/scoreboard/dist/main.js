...
const useDummyData = window.location.hostname === "localhost" ||
...
function calculateChartWidth(containerId) {
...
function renderTimeline(data) {
    ...
    const bestScoreUpdates = [];
    teams.forEach((teamData, teamName) => {
        // 時系列でソート
        const sortedData = Array.from(teamData).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    bestScoreUpdates.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    const yAxis = svg.append("g").call(d3
        .axisLeft(y)
        .tickSize(-width)
        .tickPadding(isMobile ? 5 : 10));
    ...
    teamsBestScores.forEach((teamData, teamName) => {
        ...
        const lastDataPoint = teamData.reduce((latest, current) => new Date(current.timestamp) > new Date(latest.timestamp)
            ? current
            : latest);
    ...
    svg
        ...
        tooltip
            .style("display", "block")
            .html(`Team: ${d.team}<br>Best Score: ${d.score.toLocaleString()}<br>Updated: ${timeTooltipFormat(new Date(d.timestamp))}`)
...
function renderBarChart(data) {
    ...
    const barHeight = Math.max(minBarHeight, Math.min(maxBarHeight, Math.floor(baseHeight / teamCount)));
    ...
    d3.select("#bar-chart")
        ...
        .html(`Latest Score <span style="font-size: 0.7em; color: #666; font-weight: normal;">(${teamCount} teams)</span>`);
    // Calculate width after container is created
...
function renderRankingTable(data) {
    ...
    latestScores.sort((a, b) => b.score - a.score);
    const rankings = latestScores.map((teamScore, index) => {
        return {
            rank: index + 1,
            team: teamScore.team,
    ...
    d3.select("#ranking-table")
        .append("h2")
        .style("text-align", "center")
        .html(`Ranking <span style="font-size: 0.7em; color: #666; font-weight: normal;">(${rankings.length} teams)</span>`);
    // Create table
    const table = d3
